...
@Composable
fun DementiaInspectionScreen(
    currentIndex: Int,
    totalQuestions: Int,
    @StringRes questionResId: Int,
    selectedAnswer: DementiaAnswer?,
    questionTextSize: TextUnit,
    onAnswer: (DementiaAnswer) -> Unit
) {
    Column(
        modifier = Modifier
            .fillMaxSize()
        ...
        AnswerRow(
            ...
            onClickYes = { onAnswer(DementiaAnswer.Yes) },
            onClickNo = { onAnswer(DementiaAnswer.No) }
        )

        Spacer(Modifier.height(24.dp))

        BottomCounter(index1Based = currentIndex + 1)
    }
}
...
private fun Preview_DementiaTestScreen_Mid() {
    DementiaInspectionScreen(
        ...
        selectedAnswer = DementiaAnswer.Yes,
        questionTextSize = 50.sp,
        onAnswer = {}
    )
}
