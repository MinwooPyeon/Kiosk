...
fun PresbyopiaInspectionContent(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        if (((uiState == PresbyopiaInspectionUiState.ComingCloser && ((tryCount == 0 && !isTTSSpeaking) || tryCount == 1 || tryCount == 2)) || uiState == PresbyopiaInspectionUiState.NoPresbyopia)) {
            BottomWarningButton(
                modifier = Modifier.zIndex(1F).align(if(isLandscape()) Alignment.BottomCenter else Alignment.BottomCenter).width(550.dp),
                onClick = onNextStep,
                text = nextButton,
         ...
         * 하단 거리 표시
         */
        Box(modifier = Modifier.align(if(isLandscape())Alignment.BottomEnd else Alignment.BottomCenter).padding(bottom = if(!isLandscape()) 250.dp else 100.dp)) {
            when (uiState) {
        ...
        Column(
            ...
            Box(
                ...
                modifier =
                    Modifier
                        .padding(top = 20.dp)
                        .height(if (isLandscape()) 400.dp else 500.dp)
                        .width(if (isLandscape()) 480.dp else 600.dp)
                        .background(
                            color = White,
...
@Composable
private fun PreviewPresbyopiaInspectionContentStartedLandScape() {
    val context = LocalContext.current
...
@Composable
private fun PreviewPresbyopiaInspectionContentAdjustingDistanceLandScape() {
    val context = LocalContext.current
...
@Composable
private fun PreviewPresbyopiaInspectionContentComingCloserLandScape() {
    val context = LocalContext.current
